...
import { BarChart3, Settings, Download, AlertCircle, CheckCircle, XCircle, Database, Upload, Eye, LogOut, User, GitCompare } from 'lucide-react';
...
import { ForecastComparisonDashboard } from './components/ForecastComparisonDashboard';
...
function App() {
  ...
  const [comparisonForecasts, setComparisonForecasts] = useState<ForecastResult[]>([]);
  ...
  const addForecastToComparison = (forecast: ForecastResult) => {
    // Check if forecast already exists in comparison list
    const exists = comparisonForecasts.some(f => f.configHash === forecast.configHash);
    if (!exists) {
      setComparisonForecasts(prev => [...prev, forecast]);
    }
  };

  const removeForecastFromComparison = (configHash: string) => {
    setComparisonForecasts(prev => prev.filter(f => f.configHash !== configHash));
  };
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Backend Offline Warning */}
            <div className="flex items-center space-x-4 mb-6">
              ...
              {/* Forecast Comparison Button */}
              <button
                onClick={() => setShowComparisonModal(true)}
                disabled={comparisonForecasts.length === 0}
                className="inline-flex items-center px-4 py-2 border border-orange-600 rounded-lg text-sm font-medium text-orange-600 bg-white hover:bg-orange-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <GitCompare className="w-4 h-4 mr-2" />
                Compare Forecasts ({comparisonForecasts.length})
              </button>
          ...
          <div className="space-y-6">
            ...
            {'results' in forecastResult ? (
              <MultiForecastResults 
                ...
                onSaveForecast={() => setShowSaveForecastModal(true)}
              />
            ) : (
              <ForecastResults
                ...
                onAddToComparison={addForecastToComparison}
                comparisonList={comparisonForecasts}
                onShowProcessLog={handleShowProcessLog}
              />
            )}
          </div>
        ...
        <ForecastComparisonDashboard
          isOpen={showComparisonModal}
          onClose={() => setShowComparisonModal(false)}
          forecasts={comparisonForecasts}
          onRemoveForecast={removeForecastFromComparison}
        />
