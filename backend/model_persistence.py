...
#             'externalFactors': sorted(config.get('externalFactors', []))
#         }
...
#         try:
#             print(f"Debug find_cached_model: algorithm={algorithm}, config={config}, training_data type={type(training_data)}, length={len(training_data) if training_data is not None else 'None'}")
#             config_hash = ModelPersistenceManager.generate_config_hash(config)
#             data_hash = ModelPersistenceManager.generate_data_hash(training_data)
#             model_hash = ModelPersistenceManager.generate_model_hash(algorithm, config_hash, data_hash)
            
#             print(f"Debug find_cached_model: querying database for model_hash={model_hash}")
#             saved_model = db.query(SavedModel).filter(SavedModel.model_hash == model_hash).first()
