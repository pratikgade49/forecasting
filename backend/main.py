...
FRED_API_KEY = os.getenv("FRED_API_KEY", "")  # Set this environment variable
...
class ForecastingEngine:
    ...
    def arima_forecast(data: pd.DataFrame, periods: int) -> tuple:
        """Enhanced ARIMA-like forecasting with external factors (ARIMAX)"""
        y = data['quantity'].values
        external_factor_cols = [col for col in data.columns if col not in ['date', 'quantity', 'period', 'product', 'customer', 'location']]
        ...
        if external_factor_cols:
            ...
            window = min(3, len(y) - 1)
            X, y_target = [], []
            
            for i in range(window, len(y)):
                # Autoregressive features
                ar_features = list(y[i-window:i])
                
                # Differenced features for stationarity
                if i > window:
                    diff_features = [y[i-1] - y[i-2]]
                else:
                    diff_features = [0]
                
                # External factors
                external_features = list(data[external_factor_cols].iloc[i].values)
                
                # Combined features
                features = ar_features + diff_features + external_features
                X.append(features)
                y_target.append(y[i])
            ...
            if len(X) > 1:
                ...
                for i in range(periods):
                    # Autoregressive features
                    ar_features = list(recent_values)
                    
                    # Differenced features
                    if len(recent_values) > 1:
                        diff_features = [recent_values[-1] - recent_values[-2]]
                    else:
                        diff_features = [0]
                    
                    # External factors (use last known values)
                    external_features = list(data[external_factor_cols].iloc[-1].values)
    ...
    def generate_multi_forecast(db: Session, config: ForecastConfig, process_log: List[str] = None) -> MultiForecastResult:
        ...
        if selected_dimensions == 3:
            ...
            return ForecastingEngine.generate_forecast_three_dimensions(db, config, products, customers, locations, process_log)
        else:
            if process_log is not None:
...
@app.post("/saved_forecasts", response_model=SavedForecastResponse)
async def save_forecast(
    request: SavedForecastRequest,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    """Save a forecast result for the current user"""
    try:
        # Check if forecast name already exists for this user
        ...
        if existing:
            raise HTTPException(
                status_code=400, 
                detail=f"You already have a saved forecast with the name '{request.name}'"
            )
        
        # Create new saved forecast
...
async def generate_forecast(
    ...
    try:
        ...
        # Patch ForecastingEngine.run_algorithm to disable caching if flag is False
        original_run_algorithm = ForecastingEngine.run_algorithm
        
        def run_algorithm_no_cache(algorithm, data, config_obj, save_model=True):
            return original_run_algorithm(algorithm, data, config_obj, save_model=ENABLE_MODEL_CACHE)
        
        ForecastingEngine.run_algorithm = run_algorithm_no_cache
        ...
        if config.multiSelect:
            if config.advancedMode:
                ...
                ForecastingEngine.run_algorithm = original_run_algorithm
                return result
            else:
